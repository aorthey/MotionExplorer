...
PathVisibilityChecker::PathVisibilityChecker(const base::SpaceInformationPtr &si) : si_(si)
{
...
{
public:
    pathPathValidityChecker(const ob::SpaceInformationPtr &si, const ob::SpaceInformationPtr &si_local,
                            std::vector<ob::State *> s1, std::vector<ob::State *> s2)
      : ob::StateValidityChecker(si), si_local_(si_local)
    {
        path1_ = s1;
        path2_ = s2;
        path1_length_ = 0;
    ...
    virtual bool isValid(const ob::State *state) const
    {
        ...
        if (!si_->getStateValidityChecker()->isValid(path1_interp_state_))
            return true;
        if (!si_->getStateValidityChecker()->isValid(path2_interp_state_))
            return true;
    ...
    // saves the accumulated (!) distance (distance from start to the point at this index) for each point in the
    // stateDistances vector
    // computes the overall pathLength which is equal to the last entry of stateDistances
    void computePathLength(const std::vector<ob::State *> &path, std::vector<double> &stateDistances,
                           double &pathLength)
    {
        pathLength = 0;
        if (path.size() > 1)
    ...
    void createStateAt(const std::vector<ob::State *> &path, const double &pathLength,
                       const std::vector<double> &distances, const double newPosition, ob::State *s_interpolate) const
    {
...
bool PathVisibilityChecker::IsPathVisible(const std::vector<BundleSpaceGraph::Vertex> &v1,
                                          const std::vector<BundleSpaceGraph::Vertex> &v2,
                                          BundleSpaceGraph::Graph &graph)
{
...
void PathVisibilityChecker::computePathLength(ob::SpaceInformationPtr si_, const std::vector<ob::State *> &path,
...
void PathVisibilityChecker::createStateAt(ob::SpaceInformationPtr si_, const std::vector<ob::State *> &path,
                                          const double &pathLength, const std::vector<double> &distances,
                                          const double newPosition, ob::State *s_interpolate) const
{
    assert(newPosition <= pathLength);
...
// bool PathVisibilityChecker::isStepDynamicallyFeasible(const ob::State* s_start, ob::State* s_target, const
// ompl::control::Control* c_previous, //ompl::control::Control* c_current, const double targetRegion, const
// ompl::control::SpaceInformation* siC, const //ompl::control::DirectedControlSamplerPtr sampler){
...
// ompl::control::DirectedControlSamplerPtr simpleDirectedControlSamplerAllocator(const
// ompl::control::SpaceInformationPtr siC) const {
//    unsigned int samples = 20;
//    ompl::control::DirectedControlSamplerPtr sampler =
//    std::make_shared<ompl::control::SimpleDirectedControlSampler>(siC, samples);
//    return sampler;
//}

// bool PathVisibilityChecker::IsPathDynamicallyVisible(std::vector<ob::State*> &s1, std::vector<ob::State*> &s2,
