...
{
  Triangle_mesh tmesh;
  std::string filename = "../data/terrains/DoubleLshape.off";
  std::ifstream input(filename);
  input >> tmesh;
  input.close();

  std::cout << "loaded file " << filename << " (vertices: " << num_vertices(tmesh) << ", faces: " << num_faces(tmesh) << ")" << std::endl;
  ...
  std::cout << "random face: " << target_face_index << "/" << num_faces(tmesh) << std::endl;
