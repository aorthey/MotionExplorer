...
SliceRRT::SliceRRT(const base::SpaceInformationPtr &si): RRT(si)
{
  setName("SliceRRT");
}
...
void SliceRRT::Sample(Configuration *q_random){
...
SliceRRT::Configuration* SliceRRT::Nearest(Configuration *q_random){
  return G_->nearest(q_random);
}
SliceRRT::Configuration* SliceRRT::Connect(Configuration *q_near, Configuration *q_random){
  //##############################################################################
...
bool SliceRRT::ConnectedToGoal(Configuration* q){
  double dist=0.0;
  if(q != nullptr){
...
void SliceRRT::ConstructSolution(Configuration *q_goal){
  if (q_goal != nullptr){
...
void SliceRRT::getPlannerData(base::PlannerData &data) const
{
...
ob::PlannerStatus SliceRRT::solve(const ob::PlannerTerminationCondition &ptc)
{
  Init();
