...
template <class T>
const PlannerOutput& MotionPlanner<T>::GetOutput(){
  return output;
}
template <class T>
const PlannerInput& MotionPlanner<T>::GetInput(){
  return input;
}

template <class T>
MotionPlanner<T>::MotionPlanner(RobotWorld *world_, PlannerInput& input_):
  world(world_), input(input_)
{
  ...
  if(StartsWith(algorithm.c_str(),"")) {
    if(algorithm.size()<14){
      std::cout << "Error  Algorithm: \n             Usage :<algorithm>  \n            Example: :ompl:rrt" << std::endl;
...
template <class T>
bool MotionPlanner<T>::solve(std::vector<int> path_idxs){
...
template <class T>
std::vector< std::vector<Config> > MotionPlanner<T>::GetSiblingPaths(){
...
template <class T>
void MotionPlanner<T>::UpdateHierarchy(){
...
template <class T>
bool MotionPlanner<T>::isActive(){
  return active;
}
template <class T>
void MotionPlanner<T>::DrawGLScreen(double x_, double y_){
  if(!active) return;
  viewTree.x = x_;
  viewTree.y = y_;
  viewTree.DrawGL();
...
template <class T>
void MotionPlanner<T>::DrawGL(const GUIState& state){
  if(!active) return;
  ...
  if(selected_path.size()>0){
    ...
    Node<T>* node = hierarchy.GetNode( current_path );
